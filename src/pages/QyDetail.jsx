...
class Com extends Component {
  ...
  componentDidMount (){
    ...
    api.requestQiyeData(id).then(data => {
      console.log(data);
      this.setState({
     ...
     api.requestQyTuijianData().then(data => {
          console.log(data)
          this.setState({
            list: data
          })
    ...
    onshoucang () {
     if(localStorage.getItem('isLogin') ==='ok'){
      ...
      if (arr === null) { //这是arr里面为空的时候，第一次添加
        arr=[arrData]
      } else {
        let onoff = true
        for (let i in arr) {
          if (this.state.number === arr[i].num) {
            onoff = false
          }
        }
        if ( onoff === true ) {
      ...
      <footer className="footer">
        <div className="float-col">
            <div className="sec-float-left">
              ...
              <Link to="#" className="col-wrap col-wraps">
            ...
            <div className="ri-download">
              ...
              <p to="" className="ri-open open-vip"
                  onClick={ () =>
