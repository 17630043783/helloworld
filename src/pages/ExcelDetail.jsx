...
class Com extends Component {
  ...
  componentDidMount (){
    ...
    console.log(this)
    api.requesteExcelData(id).then(data => {
      console.log(data);
      this.setState({
        dataexcel: data,
     ...
     api.requestChTuijianData().then(data => {
          console.log(data)
          this.setState({
            list: data
            // tjimg: data[0].imgurl,
    ...
    onshoucang () {
     if(localStorage.getItem('isLogin') ==='ok'){
      ...
      let data = JSON.parse(localStorage.getItem('chdata'))
      if (data === null) {
        data = [chdata]
      } else {
        let onoff = true;
        for(let i in data) {
          if (data[i].num === this.state.number) {
            onoff = false;
          }
        }
        if (onoff === true) {
      ...
      localStorage.setItem("chdata", JSON.stringify(data))
      // console.log(data)
      Toast.info('收藏成功', 1)
      } else {
        this.setState({
